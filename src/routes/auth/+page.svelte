<script lang="ts">
	...
	let step = $state(1); // 1: Credentials, 2: Vault Details (Signup only)

	// --- Vault Data (Non-critical) ---
	let email = $state('');
	let password = $state('');
	let vaultAlias = $state('');
    let baseCurrency = $state('USD');
    let burnLimit = $state(0); // 0 means no limit set
    let auditFrequency = $state('weekly'); // 'daily' | 'weekly' | 'monthly'
    
const handlePrimaryAction = () => {
        if (mode === 'signup' && step === 1) {
            step = 2;
        } else {
            // Updated to include the new registration fields
            ...
            // Logic to send to Laravel/Supabase goes here
        }
    };
</script>
...
<div class="min-h-[calc(100vh-80px)] w-full flex flex-col items-center justify-center px-6">
	<div class="w-full max-w-[400px]">
			...
			{#key mode + step}
				<div in:fade={{ duration: 200 }} class="text-center mt-4">
					<h1 class="text-2xl font-bold tracking-tight text-zinc-900">
						{#if mode === 'login'} Welcome back {:else if step === 1} Secure your Ledger {:else} Personalize your Vault {/if}
					</h1>
					<p class="text-zinc-500 text-sm mt-1.5 px-4">
						{#if mode === 'login'}
							Enter your credentials to access your ledger.
						{:else if step === 1}
							Create a secure entry point for your subscription audit.
						{:else}
							Define how you'll view your "burn" metrics.
						{/if}
					</p>
				</div>
			{/key}
		...
		<div class="bg-white border border-orange-500/20 rounded-[2.5rem] shadow-xl shadow-orange-500/5 overflow-hidden transition-all duration-500">
			<div class="p-8">
					...
					<div in:fade={{ duration: 200 }} class="space-y-4">
						...
						<div class="space-y-2">
							<div class="flex justify-between items-center px-1">
								...
								{#if mode === 'login'}
									<a href="#reset" class="text-[10px] font-bold uppercase tracking-widest text-orange-600 hover:text-orange-700">Forgot?</a>
								{/if}
							</div>
							<div class="relative group">
				...
				>
					{#if mode === 'login'} Sign In {:else if step === 1} Continue {:else} Start Your Burn {/if}
					<ArrowRight class="ml-2 size-4 group-hover:translate-x-1 transition-transform" />
				</Button>

				
				{#if mode === 'login' && step === 1}
					<div in:fade class="relative my-8">
						...
						<div class="relative flex justify-center text-[10px] uppercase tracking-tighter">
							<span class="bg-white px-3 text-zinc-400 font-medium">Or continue with</span>
						</div>
					</div>

					<div class="space-y-3">
    <div class="grid grid-cols-2 gap-3">
        <button class="flex items-center justify-center gap-2 border border-zinc-100 hover:bg-zinc-50 rounded-2xl py-3 px-4 transition-all active:scale-95 group cursor-pointer">
    ...
    Linked via Google or GitHub. We encrypt and store your email to keep your ledger synced.
    </p>
</div>
				{/if}
			</div>
    ...
    <a 
        href="/auth/register" 
        class="text-sm text-zinc-500 transition-all font-semibold active:scale-95 inline-block hover:opacity-80"
    >
        Don't have an account? 
        <span class="text-orange-600 hover:underline">Create one</span>
    </a>
</div>
		</div>

		<p class="mt-8 text-center text-[11px] leading-relaxed text-zinc-400 px-4">
			SubBurn uses zero-knowledge principles for your ledger data.
